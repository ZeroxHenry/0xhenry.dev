...
|---|---|
| 웹 검색 (트렌드/리서치) | 가능 |
| GitHub (commit/push) | 가능 |
| 파일 읽기/쓰기 | 가능 |
| SVG 코드 생성 | 가능 (텍스트 기반) |
| 이미지 생성 AI | 불가 — 사용자가 Gemini/DALL-E로 별도 생성 |

### Gemma 4.0이 할 수 있는 것
1. **스크립트 작성**: 영상 스크립트 전문 작성 가능
2. **웹 리서치**: 트렌드/경쟁 채널 분석으로 주제 선정
3. **SVG 코드 생성**: 썸네일/아이콘 SVG 코드 생성
4. **설명문/태그 작성**: YouTube 메타데이터 최적화
5. **콘텐츠 캘린더 관리**: 에피소드 기획/일정 관리
6. **기존 블로그 글 → 영상 스크립트 변환**: 웹사이트/네이버 블로그 글을 영상 대본으로 재가공
7. **Git 관리**: 작성한 콘텐츠 commit + push
