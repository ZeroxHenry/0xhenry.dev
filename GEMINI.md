...
## Operational Strategy: Tiered Reasoning

### Local by Default (Gemma 4.0)
- Use for: Code completions, bug fixes, docstrings, unit tests, routine refactors (1-3 files)
- Always attempt local `http://localhost:11434` first
- Tag responses with `[Local]` when generated by on-device model

### Cloud for Complexity
- Use for: Architectural changes, complex refactors (10+ files), security, new features
- Switch manually with `@gemini` or `@claude` when task exceeds local capabilities
