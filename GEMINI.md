...
### 컨텍스트 한계 도달 시 (Handoff Protocol)

대화가 길어져 컨텍스트가 소진되기 직전에 반드시 실행:

```bash
python3 ~/0xhenry.dev/workspaces/naver-blog/scripts/handoff_create.py \
  --summary "현재 작업 상황 요약" \
  --steps "다음 할 일 1" "다음 할 일 2" \
  --files "관련 파일 경로"
```

- 생성된 `vault/20_Meta/HANDOFF_TMP.md`가 다음 세션 시작 시 자동으로 로드되고 즉시 삭제됨.
- 인수인계 문서는 임시용 — Obsidian 지식으로 취급하지 않음.
