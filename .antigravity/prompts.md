...
## Operational Strategy: Antigravity Tiered Reasoning (Pattern 1)

This project prioritizes **Local Inference (Gemma 4.0)** for high-frequency tasks to ensure privacy, speed, and zero cost.

### 1. Local by Default (Gemma 4.0 e4b)
- **Use for**: Code completions, bug fixes, docstrings, unit tests, and routine refactors (1-3 files).
- **Behavior**: Always attempt to resolve using the local `http://localhost:11434` endpoint first.
- **Reporting**: Tag responses with `[Local]` when generated by the on-device model.

### 2. Cloud for Complexity (Claude / Gemini High)
- **Use for**: Project-wide architectural changes, complex refactors (10+ files), security threat modeling, and new feature planning.
- **Trigger**: Switch manually using `@gemini` or `@claude` when the task exceeds local capabilities.
